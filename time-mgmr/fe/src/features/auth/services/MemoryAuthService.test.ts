...
vi.mock('@/features/auth/api/authRefreshApi', () => ({
	refreshTokenRequest: mockApi.refreshTokenRequest,
}));
...
describe('MemoryAuthService', () => {
	...
	it('logout calls api with refresh token and clears auth state', async () => {
		mockApi.loginRequest.mockResolvedValue(authResponse);
	...
	it('getCurrentUser refreshes session when /me returns unauthorized', async () => {
		mockApi.loginRequest.mockResolvedValue(authResponse);
		mockApi.getCurrentUserRequest
		...
		expect(currentUser).toEqual(user);
		expect(mockApi.refreshTokenRequest).toHaveBeenCalledWith('refresh-token-1');
		expect(mockApi.getCurrentUserRequest).toHaveBeenNthCalledWith(1, {
			accessToken: 'access-token-1',
		});
		expect(mockApi.getCurrentUserRequest).toHaveBeenNthCalledWith(2, {
			accessToken: 'access-token-2',
		});
	});
});
